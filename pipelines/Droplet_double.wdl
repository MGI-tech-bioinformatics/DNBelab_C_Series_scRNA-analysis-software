...
task report {
  ...
  command {
    if [ -f ${outdir}/symbol/report_sigh.txt ];then
      ...
      echo "rm ${outdir}/temp/*fq ${outdir}/temp/*bam" >> ${outdir}/clear.sh
...
task cellCalling {
  ...
  command <<<
    if [ -f ${outdir}/symbol/cellCalling_sigh.txt ];then
      ...
      ${Rscript} ${root}/scripts/all.R -i ${outdir}/temp/cell_counts_all.tsv -o ${outdir}/report -e ${default=0 expectCell}  -f ${default=0 forceCell} 
